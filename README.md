...
***Домашнее задание N25***
***Kubernetes. Мониторинг и логирование***

В ходе домашнего задания был установлен Prometheus с помощью Helm Chart. Добавлен файл с нестандартными значениями custom_values.yml, в котором:
...
Найдены цели для сбора метрик с помощью Service Discovery и цели для мониторинга с помощью k8s API.
Для сбора метрик о потреблении ресрсов и производительности контейнеров включен сервис kube-state-metrics в Prometheus Chart.
Настроен сбор метрик со всех сервисов reddit, конфигурация job'а reddit-endpoints  разделена на 3 для каждой из компонент приложения.
Установлена Grafana с помощью Helm Chart. Добавлены дашборды из предыдущих ДЗ.
Дашборды параметризированы для просмотра разных окружений.
Для логирования развернут стек EFK, просмотрена собранная информация от наших компонентов.
